...
def gather_data_for_report(projectID, reportData, reportOptions):
    ...
    if reportOptions["includeChildProjects"]:
        ...
        projectList = []
        projectList.append(projectID)   # make in one line
    ...
    for projectID in projectList:
        ...
        for inventoryItem in inventoryItems:
            ...
            if inventoryItem["componentDescription"] != None:
                componentDescription = inventoryItem["componentDescription"].replace("\n", " - ")
                if inventoryItem["usageText"] != None:
                    componentDescription += " - " + inventoryItem["usageText"]
            else:
            ...
            if inventoryItem["component_version_id"] != None:
                componentOrigin = "Open-Source"
            else:
                componentOrigin = "Proprietary"
            ...
            #Patch Status
            patchStatus = report_data_db.get_patch_comp_version(inventoryItem["component_id"],componentVersionName)
            if patchStatus is None:
                patchStatus = "N/A"
            # Release Date: N/A
            releaseDate = "N/A"
            #    End-of-Life (EOL) Date  : N/A
            eolDate = "N/A"
            #   Criticality  
            if inventoryItem["priority"] =='1':
                criticality = "High"
            elif inventoryItem["priority"] =='2':
                criticality = "Medium"
            elif inventoryItem["priority"] =='3':
                criticality = "Low"
            else:
                criticality = "Other"
            
            # Usage Restriction
            usageRestriction = componentDescription

            checksums_hashes   = report_data_db.get_inventory_files(projectID, inventoryID)
            if inventoryItem["auditNotes"] == None:
                comments_Notes = ""
            else:
                comments_Notes = inventoryItem["auditNotes"].replace("\n", " - ")
            
            if inventoryItem["disType"] =='0':
                executableProperty = "Unknown"
            elif inventoryItem["disType"] =='1':
                executableProperty = "Internal"
            elif inventoryItem["disType"] =='2':
                executableProperty = "External"
            elif inventoryItem["disType"] =='3':
                executableProperty = "Hosted"


            archiveProperty = report_data_db.get_custom_field_value(inventoryID, "Archive Property")
            if archiveProperty is None:
                archiveProperty = "N/A"
            structuredProperty = report_data_db.get_custom_field_value(inventoryID, "Structured Property")
            if structuredProperty is None:
                structuredProperty = "N/A"

            uniqueIdentifier = "SPDXRef-Inventory-" + str(inventoryID)
            ...
            inventoryData[inventoryID] = {
                ...
                "checksums_hashes": checksums_hashes,
                "comments_Notes": comments_Notes,
                "executableProperty": executableProperty,
    ...
    reportData["topLevelProjectName"] = project_Names[0]
