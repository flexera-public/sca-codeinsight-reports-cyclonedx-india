...
def generate_json_report(reportData):
    ...
    for inventoryID in inventoryData:
        ...
        componentOrigin = inventoryData[inventoryID]["componentOrigin"]
        componentDependency = inventoryData[inventoryID]["componentDependency"]
        patchStatus = inventoryData[inventoryID]["patchStatus"]
        if not patchStatus:
            patchStatus = "N/A"
        # releaseDate = inventoryData[inventoryID]["releaseDate"]
        # eolDate = inventoryData[inventoryID]["eolDate"]
        criticality = inventoryData[inventoryID]["criticality"]
        # usageRestriction = inventoryData[inventoryID]["usageRestriction"]
        checksums_hashes = inventoryData[inventoryID]["checksums_hashes"]
        comments_Notes = inventoryData[inventoryID]["comments_Notes"]
        executableProperty = inventoryData[inventoryID]["executableProperty"]
        archiveProperty = inventoryData[inventoryID]["archiveProperty"]
        structuredProperty = inventoryData[inventoryID]["structuredProperty"]
        # uniqueIdentifier = inventoryData[inventoryID]["uniqueIdentifier"]
        component = {}
        if not bomref:
            bomref = "N/A"
        ...
        component["hashes"] = [{"alg": "MD5", "content": file_hash.get("md5", "")} for file_hash in checksums_hashes if "md5" in file_hash] if checksums_hashes else []
        component["properties"] = [{"name":"patchStatus", "value":patchStatus},{"name":"componentOrigin", "value":componentOrigin}, {"name":"criticality", "value":criticality}, {"name":"executableProperty", "value":executableProperty}, {"name":"archiveProperty", "value":archiveProperty}, {"name":"structuredProperty", "value":structuredProperty}]
        component["externalReferences"] = [{"url": componentUrl,"comment": comments_Notes,"type": "website","hashes": component["hashes"]}]
        component["licenses"] = []
        ...
        component["purl"] = purl
        if component.get("bom-ref") not in [c.get("bom-ref") for c in reportDetails["components"]]:
